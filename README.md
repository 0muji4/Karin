...
データは、本人だけのものである**記録（L1）**と、見知らぬ他人に渡る**交換（L2）**の二層に分かれる。記録は本人以外に見せたくない私的な日記でサーバを通す必要がなく、交換は見知らぬ他人に届ける以上サーバ側で害がないかを確かめてからでないと配れない。同じデータとして扱うと両立しないため、二層に分ける。

```mermaid
flowchart TD
    W[短冊を書く（自由）] --> B[文箱 L1<br/>本人だけ・サーバに暗号化保存]
